...
boolean flag = false ;
	...
	public List<Basket> getAllBasketByUserId(int userid, String isBought) throws Exception{
		...
		if (isBought == null || isBought.isEmpty()) {
			sql = "SELECT item_id, item_size, item_amount, item_sum, product_id, user_id, is_bought FROM basket WHERE user_id=? order by item_id DESC";
			this.pstmt = this.conn.prepareStatement(sql) ;
			this.pstmt.setInt(1, userid) ;
		}
		else {
			sql = "SELECT item_id, item_size, item_amount, item_sum, product_id, user_id, is_bought FROM basket WHERE user_id=? and is_bought=? order by item_id DESC";
			this.pstmt = this.conn.prepareStatement(sql) ;
			this.pstmt.setInt(1, userid) ;
			this.pstmt.setString(2, isBought);
		}
